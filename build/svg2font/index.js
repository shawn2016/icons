...
function svgFont() {
  ...
  readSrcPathSync(icon_dir).forEach((item, idx) => {
    ...
    cssString.push(`.is-icon-${name}:before { content: "${'\\'}${unicode[0].charCodeAt(0).toString(16)}";}\n`);
    cssIconHtml.push(`<li class="class-icon"><i class="is-icon-${name}"></i><p class="name">${name}</p></li>`)
